...
class YouTube {
    ...
    public function playlist_videos($playlist_id, $count = 2) {
        $result = array();
        ...
        if ($json) {
            ...
            if ($data->items) {
                ...
                for ($x = 0; $x < $count; $x++) {
                    if (!empty($data->items[$x])) {
                        $video_data = curl_get_contents("http://www.youtube.com/get_video_info?video_id=" . $data->items[$x]->contentDetails->videoId);
                        if ($video_data) {
    ...
    public function video_info($id) {
        ...
        $result = array();
        $json = curl_get_contents("http://www.youtube.com/get_video_info?video_id=" . $this->safe($id) . "");
        $video_json = curl_get_contents($url);
        ...
        if ($json and $data) {
            ...
            $result['description'] = $data->items[0]->snippet->description;
            $result['tags'] = $data->items[0]->snippet->tags;
